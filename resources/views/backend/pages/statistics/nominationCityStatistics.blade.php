            ...
            <table id="invoice" style="background-color: #f2f2f2" class="table table-bordered table-responsive">
                ...
                @foreach( $regions as $key => $r)
                    ...
                    <tr>
                        ...
                        <th class="align-middle text-center">{{  $r->city_name }}</th>
                        @foreach($personalNominations as $p => $nom)
                            <td class="align-middle  text-center">
                                {{
    $x = \Illuminate\Support\Facades\DB::table('personal_users')
    ->leftJoin('personal_user_card_information', 'personal_user_card_information.personal_user_id', '=', 'personal_users.id')
    ->where('personal_user_card_information.all_city_id', $r->id)
    ->where('nomination_id', $nom->id)->count()
     }}
