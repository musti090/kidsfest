...
class PersonalController extends Controller
{
        ...
        try {
            ...
            if ($this->user->getRoleNames()[0] == 'superadmin' || $this->user->getRoleNames()[0] == 'developer' || $this->user->getRoleNames()[0] == 'content manager') {
                ...
                $data = DB::table('personal_users')
                    ...
                    //    ->where('date', '=', '2025-04-24') // Bu şərt daima keçərli olacaq
                    /*                 ->where(function($query) {
                                         $query->where('is_absent', 1)
                                             ->orWhereNotNull('score'); // score IS NOT NULL
                                     })*/
            ...
            if ($request->get("date")) {
                $data->where("personal_user_card_information.date", $request->get("date"));
            }
            /*            if ($request->get("date") == null) {
                            $data->where("personal_user_card_information.date", null);
                        }*/
            if ($request->get("time")) {
                $data->where("personal_user_card_information.time", $request->get("time"));
            }
    ...
    public function indexTest(Request $request)
    {
        try {
            ...
            $count = $data->count();
            $data = $data->paginate(25)->appends($request->query());
            return view('backend.pages.firstStep.indexTest', compact('data', 'count', 'nominations', 'cities', 'regions', 'nominations_data', 'regions_data', 'cities_data', 'user_precinct', 'precinct_data'));
    ...
    public function exportExcel(Request $request)
    {
        try {
            ...
            return Excel::download(new PersonalExport($data, $cities, $regions, $nominations, $education_schools, $education_school_names, $education_new_school_names, $precincts), 'Fərdi iştirakçılar.xlsx');
        } catch (\Exception $e) {
