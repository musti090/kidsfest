...
class SearchController extends Controller
{
    ...
    public function timePlacePersonal(Request $request)
    {
        $user_data = [];
        try {
            ...
            if ($request->has('document') && $request->document == "fin_code") {
                if ($request->value != null) {
                    ...
                    if ($fin_code->exists()) {
                        ...
                        $user_data['ad_soyad'] = $data->name . " " . $data->surname;
                        $user_data['kod'] = $data->UIN;
                        $user_data['rayon'] = $this->getRegionName($data->mn_region_id);
                        $user_data['nominasiya'] = $this->getNominationName($data->nomination_id);
                        $user_data['gun'] = $this->getDate($data->date);
                        $user_data['vaxt'] = $this->getTime($data->time);
                        $user_data['mekan'] = $this->getPlace($data->precinct_id);
                        DB::table('personal_users_check')->insert([
                            'name' => $data->name,
                            'surname' => $data->surname,
                            'UIN' => $data->UIN,
                            'fin_code' => $data->fin_code,
                            'mn_region_id' => $data->mn_region_id,
                ...
                $id_code = DB::table('personal_user_card_information')
                    ->leftJoin('personal_users', 'personal_user_card_information.personal_user_id', '=', 'personal_users.id')
                    ->where('UIN', $request->value);
                if ($id_code->exists()) {
                    $data = $id_code->first();
                    if ($data->date == null || $data->time == null) {
                        return response(['message' => 'Hörmətli iştirakçı, müraciətinizə görə təşəkkür edirik! Böyük sorğu sayını nəzərə alaraq sistemdə uyğunlaşdırma prosesi aparılır. Sizin vaxt və məkan məlumatlarınız qısa zaman ərzində təqdim olunacaqdır. Məlumatı bir neçə saatdan sonra yenidən yoxlamağınız xahiş olunur.']);
    ...
    public function timePlaceCollective(Request $request)
    {
        $user_data = [];
        try {
            ...
            if ($request->has('type') && $request->type == "collective") {
                ...
                if ($request->has('document') && $request->document == "col_fin_code") {
                    if ($request->value != null) {
                        $fin_code = DB::table('collective_directors')
                            ->leftJoin('collectives', 'collectives.id', '=', 'collective_directors.collective_id')
                            ->where('director_fin_code', strtoupper($request->value));
                        ...
                        if ($fin_code->exists()) {
                            ...
                            $user_data['ad_soyad'] = $data->director_name . " " . $data->director_surname;
                            $user_data['kollektivin_adi'] = $data->collective_name;
                            $user_data['kod'] = $data->UIN;
                            $user_data['rayon'] = $this->getRegionName($data->collective_mn_region_id);
                            $user_data['nominasiya'] = $this->getNominationName($data->collective_nomination_id);
                            $user_data['gun'] = $this->getDate($data->date);
                            $user_data['vaxt'] = $this->getTime($data->time);
                            $user_data['mekan'] = $this->getPlace($data->precinct_id);
                            DB::table('collective_users_check')->insert([
                                'director_name' => $data->director_name,
                                'director_surname' => $data->director_surname,
                                'collective_name' => $data->collective_name,
                                'UIN' => $data->UIN,
                                'director_fin_code' => $data->director_fin_code,
                                'collective_mn_region_id' => $data->collective_mn_region_id,
                                'collective_nomination_id' => $data->collective_nomination_id,
                                'date' => $data->date,
                                'time' => $data->time,
                                'precinct_id' => $data->precinct_id,
                                'created_at' => now(),
                    ...
                    $id_code = DB::table('collective_directors')
                        ->leftJoin('collectives', 'collectives.id', '=', 'collective_directors.collective_id')
                        ->where('UIN', $request->value);
                    if ($id_code->exists()) {
                        $data = $id_code->first();
    ...
    public function timePlace(Request $request)
    {
        ...
        try {
            ...
            if ($request->has('type') && $request->type == "personal") {
                if ($request->has('document') && $request->document == "fin_code") {
                    if ($request->value != null) {
                        $fin_code = PersonalUser::query()->where('fin_code', $request->value);
                        if ($fin_code->exists()) {
                            $data = $fin_code->with('personal_user_form_information')->first();
                            if ($data->date == null || $data->time == null) {
                                return response(['message' => 'Hörmətli iştirakçı, müraciətinizə görə təşəkkür edirik! Böyük sorğu sayını nəzərə alaraq sistemdə uyğunlaşdırma prosesi aparılır. Sizin vaxt və məkan məlumatlarınız qısa zaman ərzində təqdim olunacaqdır. Məlumatı bir neçə saatdan sonra yenidən yoxlamağınız xahiş olunur.']);
                            }
    ...
    {
        if ($id != null) {
            $place = Precinct::select('id', 'place_name', 'place_address')->where('id', $id)->firstOrFail();
            $pa = [];
